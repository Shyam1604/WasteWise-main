...
const MapScreen = ({ route }) => {
              ...
              if (result.cancelled || result.canceled) {
                ...
                if (uri) {
                  try {
                    ...
                    console.log("this is garbage",isGarbage)
                    if (isGarbage) {
                      // Set the image URI and show the type selection modal
                      
                      setBinImageUri(uri);
                      setTypeModalVisible(true);
                    } else {
                      // Show an alert if the image is not recognized as a garbage-related item
                      Alert.alert(
                        'Image Analysis',
  ...
  //     const storageRef = ref(FIREBASE_STORAGE, `bin_image/${filename}`);
...
// this is a correct upload
  const uploadImageToFirebase = async (uri) => {
    ...
    try {
      ...
      const resizedUri = manipulatedImage.uri;
      const filename = resizedUri.substring(resizedUri.lastIndexOf('/') + 1);  // Get the filename
      const uploadUri = Platform.OS === 'ios' ? resizedUri.replace('file://', '') : resizedUri;

      // Reference to Firebase Storage
      const storageRef = ref(FIREBASE_STORAGE, `bin_image/${filename}`);
      const img = await fetch(uploadUri);
      const bytes = await img.blob();  // Convert to Blob for uploading
      console.log(img)
  ...
  console.log(uploadBytes)
      ...
      await saveImageMetadataToFirestore(downloadUrl, filename);
...
//         // Upload Base64 directly to Firebase
//         const storageRef = ref(FIREBASE_STORAGE, `bin_image/${filename}`);
...
//         // Save metadata in Firestore (optional, replace with your logic)
//         await saveImageMetadataToFirestore(downloadUrl, filename);
...
// };
  ...
  // Function to save image metadata (like URL) to Firestore
  const saveImageMetadataToFirestore = async (downloadUrl, filename) => {
    try {
      // Reference to Firestore collection 'bin_image'
      const binImagesCollectionRef = collection(FIREBASE_DB, 'bin_image');
  
      // Add metadata to Firestore
      const docRef = await addDoc(binImagesCollectionRef, {
